...
Doc.prototype.ingestSnapshot = function(snapshot, callback) {
  ...
  var doc = this;
  // A type-null snapshot means the doc doesn't exist server-side (never
  // created, or deleted). Two distinct cases:
  //
  // - No local copy: skip persisting. A missing row already communicates
  //   "doesn't exist," and writing an empty row creates a race: doc.create()
  //   defers its persist to nextTick, so readers can briefly see a row with
  //   null data fields.
  // - Stale local copy (doc is in the DurableStore inventory): the doc was
  //   deleted server-side while this client wasn't subscribed. The deletion
  //   MUST be persisted, overwriting the stale record with a type-null one
  //   (the same shape the subscribed del path writes via _handleOp).
  //   Otherwise the old payload survives in local storage forever: cache-first
  //   readers (ensureDocHasData) would restore it as if alive, and raw-SQL
  //   projections would keep surfacing it.
  if (!this.type) {
    var isStaleLocalCopy = this.durableStore &&
      this.durableStore.isDocInInventory(this.collection, this.id);
    if (!isStaleLocalCopy) {
      doc.emit('load');
      return callback && setTimeout(callback);
    }
    this.emit('enqueue persist', {reason: 'ingestSnapshot, deleted doc with stale local copy'});
    return this._putToDurableStore(function() {
      doc.emit('load');
      callback && callback();
    });
  }
...
// True if a json1 op contains a "value-less" remove (`{r: true}`): a remove
// whose captured content was stripped — which is what a transform under
// PermissiveJson1's RM_UNEXPECTED_CONTENT tolerance produces when two
// clients concurrently replace the same path (constant on `meta.updated_at`
// of shared docs). Such an op is fine to APPLY, but its INVERSE is lossy:
// json1 invert({r:true, i:X}) === {r:X, i:true}, i.e. it inserts the literal
// boolean `true` where the old value used to be. Rolling back with that
// inverse corrupts the doc locally (observed 2026-09-03: `meta.updated_at
// .utc_time === true` → DurableStore "boolean version" refusals → stale doc →
// further rejections → loop; three such `true`s even reached the server).
function opHasLossyRemove(op) {
  if (Array.isArray(op)) {
    for (var i = 0; i < op.length; i++) {
      if (opHasLossyRemove(op[i])) return true;
    }
    return false;
  }
  if (op && typeof op === 'object') {
    if (op.r === true) return true;
    for (var key in op) {
      if (opHasLossyRemove(op[key])) return true;
    }
  }
  return false;
}
...
Doc.prototype._rollback = function(err) {
  ...
  // An invertible type is not enough: the inverse must also be FAITHFUL.
  // A value-less remove inverts to "insert `true`", so for such ops a local
  // invert would corrupt the doc — re-fetch the server's snapshot instead.
  if (opHasLossyRemove(op.op)) {
    return this._hardRollback(err);
  }
